 ...
 *
 * @module @maw/dashboard/playwright.config
 */
...
export default defineConfig({
  ...
  // Skip local servers when testing against a deployed URL
  ...(isDeployed
    ? {}
    : {
        webServer: [
          // Dashboard dev server — always runs for local testing
          {
          ...
          // Agent server — only for local integration tests
          ...(runIntegration
            ? [
                {
