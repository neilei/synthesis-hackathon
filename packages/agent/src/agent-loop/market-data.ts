...
export async function gatherMarketData(
  ...
  const priceResult: Record<string, unknown> = { price: ethPrice.price, citation: ethPrice.citation };
  ...
  intentLogger?.log("price_fetch", { cycle, tool: "coinmarketcap", duration_ms: Date.now() - startPrice, result: priceResult });
